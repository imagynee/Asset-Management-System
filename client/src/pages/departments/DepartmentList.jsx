    ...
    <div className="animate-fade-in space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {departments.map((department) => (
            <Card key={department._id} className="p-5 transition hover:shadow-md">
              <div className="mb-3 flex items-start justify-between gap-2">
                <div className="inline-flex rounded-lg bg-brand-50 px-3 py-1 text-xs font-semibold text-brand-700">
                  Department
                </div>
              ...
              <h3 className="text-lg font-semibold text-slate-900">{department.deptName}</h3>
              {department.deptIncharge && (
                <p className="mt-1 text-sm text-slate-500">In charge: {department.deptIncharge}</p>
              )}
              {department.additionalNote && (
                <p className="mt-2 text-sm text-slate-500">{department.additionalNote}</p>
              )}
              <div className="mt-4 flex justify-end">
                <span className="inline-flex h-8 min-w-8 items-center justify-center rounded-lg bg-violet-50 px-2 text-sm font-bold text-violet-700 ring-1 ring-inset ring-violet-600/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <FormField label="Department In-charge">
            <TextInput
              ...
              onChange={(e) => setForm({ ...form, deptIncharge: e.target.value })}
            />
          </FormField>
          <FormField label="Additional Notes">
            <TextArea
              ...
              onChange={(e) => setForm({ ...form, additionalNote: e.target.value })}
            />
          </FormField>
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete Department" size="sm">
        <p className="text-sm text-slate-600">
          Are you sure you want to delete <strong>{deleteTarget?.deptName}</strong>? This cannot be
          undone if the department has no linked assets.
        </p>
